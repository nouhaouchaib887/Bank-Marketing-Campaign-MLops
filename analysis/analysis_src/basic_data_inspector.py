...
class CategoricalValuesInspectionStrategy(DataInspectionStrategy):
    def inspect(self, df: pd.DataFrame):
        """
        Prints unique values and their counts for each categorical feature.

        Parameters:
        df (pd.DataFrame): The dataframe to be inspected.

        Returns:
        None: Prints unique values and counts to the console.
        """
        ...
        for col in categorical_cols:
            ...
            print(df[col].value_counts().head(10))  # show top 10 values for readability
